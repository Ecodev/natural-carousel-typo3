    ...
    $(function() {
        ...
        function initGallery() {
            ...
            for (var i = 0; i < infinitesScrollGallery.length; i++) {
                ...
                gallery.rootElement = $('#tx-carouselgallery-' + gallery.id);
                gallery.bodyElement = gallery.rootElement.find('.tx-carouselgallery-body');
        ...
        function addElements(gallery, rows) {
            ...
            if (gallery.pswpContainer.length === collection.length) {
                gallery.rootElement.find('.tx-carouselgallery-next').hide();
                return;
            }
            ...
            gallery.rootElement.find('.tx-carouselgallery-noresults').hide();
            gallery.rootElement.find('.tx-carouselgallery-numberOfVisibleImages').text(gallery.pswpContainer.length);
            gallery.rootElement.find('.tx-carouselgallery-totalImages').text(collection.length);
        ...
        function filterByTerm(gallery) {
            ...
            var term = removeDiacritics(gallery.rootElement.find('.tx-carouselgallery-searchTerm').val()).toLowerCase();
            var filteredImages = [];
        ...
        function filterByCategory(gallery) {
            ...
            gallery.rootElement.find('.tx-carouselgallery-category:checked').each(function() {
        ...
        function reset(gallery) {
            ...
            gallery.rootElement.find('.tx-carouselgallery-noresults').show();
        }
        ...
        function getGallery(element) {
            ...
            if (element) {
                var galleryId = $(element).parents('.tx-carouselgallery').data('galleryid');
         ...
         */
        $('.tx-carouselgallery-next a').on('click', function(e) {
            e.preventDefault();
         ...
         */
        $('.tx-carouselgallery-category').on('change', function(e) {
         ...
         */
        $('.tx-carouselgallery-searchTerm').on('keydown', function(event) {
        ...
        $('.tx-carouselgallery-searchTerm').on('blur', function() {
            var gallery = getGallery(this);
            if (gallery.lastSearch != $(this).val()) {
                gallery.lastSearch = $(this).val();
                filterSelection(gallery);
        ...
        if (infinitesScrollGallery.length == 1 && getGallery().limit === 0) {
            ...
            $(document).scroll(function() {
                ...
                if (scroll_delta > 0 && $(window).scrollTop() > endOfGalleryAt) {
                    ...
                    //var numberOfVisibleImages = parseInt($("#tx-carouselgallery-numberOfVisibleImages").html());
                    //var totalImages = parseInt($("#tx-carouselgallery-totalImages").html());
